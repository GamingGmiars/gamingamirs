-- Ranxware v2: Restored | Murderers VS Sheriffs Duels
-- (RANXWARE SCRIPT HUB IS ONLY IN ALPHA PHASE AND IS UNFINISHED! EXPECT MANY BUGS WHEN EXPERIENCING WITH THIS UTILITY!)

if not shared.ranxwareExecuted then
	game:Shutdown()
end
...
local window = lib:CreateWindow({
	Name="Ranxware V2 [ALPHA] | MVS Duels",
	LoadingTitle="MVS Duels Script Utility",
	LoadingSubtitle="by: Ranxware V2",
	ConfigurationSaving={
...
task.spawn(function()
	local hitboxes = {CurrentValue = true}
...
task.spawn(function()
	...
	local silentAimFOVSize = {CurrentValue = 20}
	local silentAimFOVVisible = {CurrentValue = false}
	...
	local silentAimHooker;silentAimHooker = hookmetamethod(game,"__namecall",function(self,...)
		...
		if silentAim.CurrentValue and not checkcaller() and tostring(self) == "Shoot" and getnamecallmethod() == "FireServer" then
			...
			if nearest then
				...
				silentAimParams.FilterDescendantsInstances = {customStore.you.char, unpack(funcs.getAllies())}
...
task.spawn(function()
	local holding = false
	local autoClicker = {CurrentValue = false}
	autoClicker = combatTab:CreateToggle({
		Name = "Stab Autoclicker",
		CurrentValue = false,
		Callback = function(value)
			if value then
				ranxConnections:BindConnection("AutoclickerHoldingBegan",game:GetService("UserInputService").InputBegan:Connect(function(input,gpe)
					if gpe then return end
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						holding = true
					end
				end))
				ranxConnections:BindConnection("AutoclickerHoldingEnded",game:GetService("UserInputService").InputEnded:Connect(function(input,gpe)
					if gpe then return end
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						holding = false
					end
				...
				task.spawn(function()
					repeat
						if holding and funcs.isAlive(customStore.you.char) then
						...
						task.wait()
					until not autoClicker.CurrentValue
				end)
			else
				ranxConnections:UnbindConnection("AutoclickerHoldingBegan")
				ranxConnections:UnbindConnection("AutoclickerHoldingEnded")
			end
		end
	})
end)
...
local usingPropellerAnim = false
task.spawn(function()
			...
			if value then
				task.spawn(function()
					repeat
						local revolver = funcs.getRevolver()
						if revolver then
							if not usingPropellerAnim then
					...
					if customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicRoundEnded.CurrentValue and string.lower(result) == "won" then
								funcs.sayMessage(autoToxicRoundEndedPhrase ~= "" and autoToxicRoundEndedPhrase or "ez win LLL | ranxware v2")
							end
						end
					end
				end))
					...
					if customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicMatchEnded.CurrentValue and string.lower(result) == "won" then
								funcs.sayMessage(autoToxicMatchEndedPhrase ~= "" and autoToxicMatchEndedPhrase or "EZZZZZ | ranxware v2")
							end
						end
					end
				end))
					...
					if owner == customStore.you.lplr and customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicPlayerKilled.CurrentValue and funcs.getTeamStatus(player) == "Enemy" then
								local message = autoToxicPlayerKilledPhrase ~= "" and autoToxicPlayerKilledPhrase or "L <name> | ranxware v2"
								if message then
...
task.spawn(function()
	...
	local boxESP = {CurrentValue = true}
...
task.spawn(function()
	local nametagsTable = {}
	local tagTexts = {}
	local nametagsESP = {CurrentValue = true}
		...
		for username,tag in next,nametagsTable do
			...
			if player and player.Character and player.Character.Parent == workspace then
				...
				if root and head then
					local rootPosition,rootVisible = WorldToViewportPoint(root.Position)
					local headPosition = WorldToScreenPoint((head:GetRenderCFrame() * CFrame.new(0,head.Size.Y + root.Size.Y,0)).Position)
					if not rootVisible then
						tag.Visible = false
						continue
					end
...
task.spawn(function()
	local spamKnifeThrow = {CurrentValue = false}
	spamKnifeThrow = miscTab:CreateToggle({
		Name = "Knife-Throw Spammer",
		CurrentValue = false,
		Callback = function(value)
			if value then
				task.spawn(function()
					repeat
						...
						if knife then
							for _,plr in next,funcs.getEnemies() do
								if funcs.isAlive(plr.Character) then
									local suc,position = pcall(function() return plr.Character:FindFirstChild("HumanoidRootPart").Position end)
									if suc then
										customStore.remotes.throwStartEvent:FireServer(unpack({
											[1] = position,
											[2] = Vector3.new(0,-1,0)
										}))
									end
						...
						task.wait()
					until not spamKnifeThrow.CurrentValue
				end)
			end
		end
	})
...
-- whitelisted player join notifier:
...
-- ranxware whitelist chat:
-- secret admin commands shhh (you can't bypass them lol)
local commands = {}
commands.kill = function()
	local char = customStore.you.char
	if not currentlyRespawning and funcs.isAlive(char) then
		customStore.you.hum.Health = -9e9
	end
end
...
commands.punish = function()
	local char = customStore.you.char
	local hum = customStore.you.hum
	if not currentlyRespawning and char then
		char.Parent = nil
	end
end
...
game:GetService("TextChatService").OnIncomingMessage = function(message)
	...
	if message.TextSource then
		...
		if player then
			local isAdmin,tag = whitelist.getWhitelistedUser(player)
			if isAdmin then
				props.PrefixText = '<font color="#'..tag.Color:ToHex()..'">['..tag.Title..']</font> '..message.PrefixText
					...
					if commands[string.sub(string.lower(command),2)] then
						if target then
							if string.lower(target) == "all" then
								if whitelist.ClientPriority == 0 then
...
-- ranxware user detection: [testing]
ranxConnections:BindToHeartbeat("RanxwareUserDetection",function()
	for _,plr in next,game.Players:GetPlayers() do
		if whitelist.ClientPriority > 0 then
			if plr ~= customStore.you.lplr then
				local whitelisted = whitelist.getWhitelistedUser(plr)
				if not whitelisted then
					if plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChildOfClass("Humanoid").HipHeight > 2.4 and not table.find(shared.ranxUsers, plr) then
						table.insert(shared.ranxUsers, plr)
						lib:Notify({
							Title = "Ranxware V2 Whitelist",
							Content = "@" .. plr.Name .. " has been detected using Ranxware V2!",
							Duration = 8,
						})
					end
				end
			...
			local whitelisted = whitelist.getWhitelistedUser(plr)
			if plr ~= customStore.you.lplr and whitelisted then
				if not currentlyRespawning then
					customStore.you.hum.HipHeight = 2.5
				end
			end
		end
	end
